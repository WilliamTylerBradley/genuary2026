...
def grow_segment(start_point, start_control_point):
    ...
    end_control_point = Point(end_point.x + end_control_r * np.cos(end_control_theta),
                              end_point.y - end_control_r * np.sin(end_control_theta))    
...
def grow_flower(start_point):
    ...
    for step in np.arange(10):
        ...
        # create point class and pass it around


        petal_end_r = 5 * np.sqrt(rng.uniform())
        petal_end_theta = rng.uniform() * 2 * np.pi

        petal_end_point = Point(start_point.x + petal_end_r * np.cos(petal_end_theta),
                                start_point.y - petal_end_r * np.sin(petal_end_theta))

        petal_width = rng.uniform(.25, 1.75)

        end_control_point_1 = Point(-1 * (start_point.y - petal_end_point.y) * petal_width + petal_end_point.x,
                                    (start_point.x - petal_end_point.x) * petal_width + petal_end_point.y)

        end_control_point_2 = Point(-1 * (end_control_point_1.x - petal_end_point.x) + petal_end_point.x,
                                    -1 * (end_control_point_1.y - petal_end_point.y) + petal_end_point.y)

        flower.append(svg.Path(d=[svg.M(start_point.x, start_point.y),
                        svg.C(start_point.x, start_point.y,
                              end_control_point_1.x, end_control_point_1.y,
                              petal_end_point.x, petal_end_point.y),
                        svg.C(end_control_point_2.x, end_control_point_2.y,
...
def create_growth():
    ...
    while segments:
        ...
        segment = svg.Path(d=[svg.M(s.start_point.x, s.start_point.y),
                            svg.C(s.start_control_point.x, s.start_control_point.y,
                                  s.end_control_point.x, s.end_control_point.y,
        ...
        segment = svg.Path(d=[svg.M(s.start_point.x, s.start_point.y),
                            ...
                            stroke_width=(s.start_control_point.y / HEIGHT * 3)-.5)
        growth.append(segment)
        ...
        if s.end_point.y > 0:
            ...
            start_control_point = Point(-1 * (s.end_control_point.x - s.end_point.x) + s.end_point.x,
                                        -1 * (s.end_control_point.y - s.end_point.y) + s.end_point.y)
